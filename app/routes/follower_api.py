...
following_api = Blueprint('following_api',__name__)
...
class FollowApi(MethodView):
    ...
    def delete(self,username):
        ...
        follow = Follow.query.filter_by(follower_id = current_user.id , following_id = user_to_unfollow.id).first()
        if not follow:
            return jsonify({"detail": f"You are not following {username}"}), 400   
        
        db.session.delete(follow)
        db.session.commit()
        reverse_follow = Follow.query.filter_by(follower_id=user_to_unfollow.id, following_id=current_user.id).first()
        if reverse_follow:
            return jsonify({"detail": f"You have unfollowed {username}, but they are still following you."}), 200
        else:
            return jsonify({"detail": f"You have unfollowed {username} and they are no longer following you."}), 200
...
follower_api.add_url_rule(
    "/api/follow/", view_func=follower_view, methods=["POST"]
)
follower_api.add_url_rule(
    "/api/follow/<username>/", view_func=follower_view, methods=["DELETE"]
)
follower_api.add_url_rule(
    "/api/follower/", view_func=follower_view, methods=["GET"]
)


following_view = FollowingApi.as_view('following_api')
following_api.add_url_rule(
    "/api/following/", view_func=following_view, methods=["GET"]
)
