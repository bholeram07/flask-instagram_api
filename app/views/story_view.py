...
class UserStory(MethodView):
    ...
    def get(self, story_id):
        ...
        if str(story.story_owner) != str(self.current_user_id):
            story_view = StoryView.query.filter_by(viewer_id = self.current_user_id).first()
            if not story_view:
    ...
    def delete(self,story_id):
        ...
        story = Story.query.filter_by(id = story_id,is_deleted = False,story_owner = self.current_user_id).first()
        if not story:
